...
(function() {
    ...
    $(".bodyconmenu.close").prev(".body_con_left_div h3").css("background-image", "url('images/arrow.png')");

    for (var key in localStorage) {
        if (localStorage[key] == "trueLike") {
            $("." + key).children("img").attr("src", "images/heart.png");
            $("." + key).children("img").next("br").next("span").text("הסר");
            document.cookie = key + "=trueLike; " + expires;
        } else if (localStorage[key] == "falseLike") {
            $("." + key).children("img").attr("src", "images/heart_nofill.png");
...
$(".favo").click(function() {
    ...
    for (var key in localStorage) {
        if (localStorage[key] == "trueLike") {
            $("." + key).children(".favo").attr("src", "images/heart.png");
            document.cookie = favoName + "=trueLike; " + expires;
            $("." + key).children("img").next("br").next("span").text("הסר");
        } else if (localStorage[key] == "falseLike") {
            $("." + key).children(".favo").attr("src", "images/heart_nofill.png");
